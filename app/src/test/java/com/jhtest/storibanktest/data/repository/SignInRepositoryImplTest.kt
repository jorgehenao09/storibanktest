...
class SignInRepositoryImplTest {
    ...
    @Test
    fun `signInUser should emit success when firebaseAuthDataSource returns a user`() =
        runTest {
    ...
    @Test
    fun `signInUser should emit failure when firebaseAuthDataSource throws an exception`() =
        runTest {
            val email = "test@example.com"
            val password = "password123"
