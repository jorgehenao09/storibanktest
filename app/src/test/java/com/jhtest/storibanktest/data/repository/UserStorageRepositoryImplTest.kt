...
class UserStorageRepositoryImplTest {
    ...
    @Test
    fun `isUserLogged should emit success when user is logged`() = runTest {
        val userLogged = true
        val userPreferencesProto = mockk<UserPreferencesProto>(relaxed = true) {
    ...
    @Test
    fun `isUserLogged should emit failure when an exception is thrown`() = runTest {
    ...
    @Test
    fun `saveUserData should save user data successfully`() = runTest {
        val userId = "12345"
        val name = "John"
        val lastName = "Doe"
